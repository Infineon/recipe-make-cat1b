...
ifeq ($(APPTYPE),ram)
_MTB_RECIPE__OPENOCD_PROGRAM_IMG=$(MTB_RECIPE__LAST_CONFIG_DIR)/$(APPNAME).bin
else
...
ifneq ($(filter NON_SECURE,$(VCORE_ATTRS)),)
_MTB_RECIPE__PREBUILT_SECURE_APP_PROGRAM_CMD:=flash write_image erase $(_MTB_RECIPE__PREBUILT_SECURE_APP)
endif
endif
ifeq ($(VS_ERASE),1)
ifeq ($(SS_CONFIG),1)
...
ifneq (,$(_MTB_RECIPE__IS_DIE_PSC3))
ifneq ($(filter NON_SECURE,$(VCORE_ATTRS)),)
_MTB_RECIPE_OPENOCD_PROGRAM=init; reset init; flash write_image erase $(_MTB_RECIPE__OPENOCD_PROGRAM_IMG); $(_MTB_RECIPE__PREBUILT_SECURE_APP_PROGRAM_CMD); reset run; shutdown;
else
